...
window.convertObjToString = function (obj) {
  ...
  return Object.values(obj).reduce(function (acc, crr) {
    return acc += crr + separator;
  }, '');
};
...
window.redirectTo = function (endpoint) {
  return window.location.href = UISelect.baseUrl() + endpoint;
};
...
var clickDataSellers = function clickDataSellers() {
  // let sellers = Array.from(UISelect.dataSellers());
  var sellers = UISelect.dataSellers();
  sellers.forEach(function (item) {
    return item.onclick = function (e) {
      ...
      history.pushState({
        page: clickedPage
      }, "Vendedores - pág: " + clickedPage, "?page=" + clickedPage);
      fetchSellersResult();
    };
  });
};
